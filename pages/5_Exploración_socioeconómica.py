# ==========================================
# 5_Exploración_socioeconómica.py — versión sincronizada con header
# ==========================================
...
if mostrar_decadas and not df_filtrado.empty:
    st.markdown("---")
    with st.expander("📊 Media de emisiones por década", expanded=False):
...
if mostrar_prediccion and not df_filtrado.empty:
    st.markdown("---")
    with st.expander("🔮 Predicción de emisiones hasta 2100", expanded=False):
...
if not df_filtrado.empty:
    ...
    pendiente = coefg[0] if "coefg" in locals() else 0
...
with col2:
    try:
        ...
        st.download_button("🌐 Descargar HTML interactivo",
